...
import { buildChatMemory } from '../utils/memoryBuilder';
import { buildHairSalonPrompt } from '../prompts/buildHairSalonPrompt';


export async function sendMessageToGPT(userMessage, businessData, messages)
 {
    const today = new Date().toISOString().split('T')[0]; // yyyy-mm-dd
    const chatMemory = buildChatMemory(messages);
    const systemPrompt = buildHairSalonPrompt(businessData, today, chatMemory);
