...
export function buildHairSalonPrompt(
  ...
  const formattedServices = services.length
    ? services
        .map(
          (s, i) =>
            `${toHebrewLetter(i + 1)}. ${s.name} – ${s.price} ש"ח (משך: ${s.duration} דקות)`
        )
        .join("\n  ")
    : "אין שירותים מוגדרים.";
...
2. אם לא צויין תאריך – אל תנחש. שאל את הלקוח לאיזה תאריך ושעה הוא מעוניין לקבוע תור.
...
12. אם המשתמש שואל על תור ליום מסוים ללא ציון טווח זמן (בוקר/צהריים/ערב) – צור בלוק check_availability עם שדות: service + date בלבד (ללא time_preference).
13. אם המשתמש שואל שוב על תור ליום אחר או זמן אחר (למשל "מה לגבי מחר בצהריים?") – צור בלוק חדש מסוג check_availability בהתאם להקשר, עם השדות: service, date ו־time_preference אם רלוונטי.

📦 דוגמה לבדיקת זמינות (כוללת time_preference):
\`\`\`json
{
...
📦 דוגמה לבדיקת זמינות בלי time_preference:
\`\`\`json
{
  "action": "check_availability",
  "date": "2025-04-24",
